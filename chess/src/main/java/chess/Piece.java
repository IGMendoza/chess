...
public class Piece {
	...
	public boolean hasMoved() {
		boolean result = this.hasMoved == 0 ? false : true;
		return result;
	}
	
	public void increaseHasMoved() {
		this.hasMoved++;
	}
	...
	public boolean move(String n_position, boolean capture, ArrayList<Piece> allies, 
													ArrayList<Piece> enemies, int turn) {
		int forward;
		int forward_2;
		int horizontal;
		int vertical;
		...
		case "Queen":
			...
			if(Math.abs(slope) != 1 && Math.abs(slope) != 0) {
				if(n_position.charAt(0) - this.position.charAt(0) != 0) {
					return false;
				}
			}
			...
			if(this.hasMoved == 0 && (n_position.charAt(0) == this.position.charAt(0) + 2 || 
				...
				for(int i = 0; i < allies.size(); i++) {
					if(this.position.equals("e1") && n_position.equals("c1")) {
						...
						if(allies.get(i).getName().equals("wr1")) {
							rook_idx = i;
						}
					}
					if(this.position.equals("e1") && n_position.equals("g1")) {
						...
						if(allies.get(i).getName().equals("wr2")) {
							rook_idx = i;
						}
					}
					if(this.position.equals("e8") && n_position.equals("c8")) {
						...
						if(allies.get(i).getName().equals("br1")) {
							rook_idx = i;
						}
					}
					if(this.position.equals("e8") && n_position.equals("g8")) {
						...
						if(allies.get(i).getName().equals("br2")) {
							rook_idx = i;
						}
					}
				}
				for(int i = 0; i < enemies.size(); i++) {
					if(this.position.equals("e1") && n_position.equals("c1")) {
						if(enemies.get(i).getPosition().equals("b1") ||
								enemies.get(i).getPosition().equals("d1")) {
							done_goofed = true;
						}
					}
					if(this.position.equals("e1") && n_position.equals("g1")) {
						if(enemies.get(i).getPosition().equals("f1")) {
							done_goofed = true;
						}
					}
					if(this.position.equals("e8") && n_position.equals("c8")) {
						if(enemies.get(i).getPosition().equals("b8") ||
								enemies.get(i).getPosition().equals("d8")) {
							done_goofed = true;
						}
					}
					if(this.position.equals("e8") && n_position.equals("g8")) {
						if(enemies.get(i).getPosition().equals("f8")) {
							done_goofed = true;
						}
					}
				}
				// If Castling possible, move both pieces
				if(!done_goofed) {
					// Move King if Rook hasn't been moved
					if(allies.get(rook_idx).hasMoved()) {
						return false;
					}
					this.position = n_position;
					this.hasMoved++;
					...
					rook_position = n_position.equals("c1") ? "d1" : rook_position;
					rook_position = n_position.equals("g1") ? "f1" : rook_position;
					rook_position = n_position.equals("c8") ? "d8" : rook_position;
					rook_position = n_position.equals("g8") ? "f8" : rook_position;
					
					allies.get(rook_idx).setPosition(rook_position);
					allies.get(rook_idx).increaseHasMoved();
					return true;
				}
			}
			// Regular moves
			if(n_position.charAt(0) == this.position.charAt(0) + horizontal &&
					n_position.charAt(1) == this.position.charAt(1) + vertical) {
				this.position = n_position;
				this.hasMoved++;
				return true;
			}
